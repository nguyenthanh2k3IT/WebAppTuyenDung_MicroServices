...
function UpdateCategoryModal() {
    ...
    const [updateCategoryInfo, setUpdateCategoryInfo, setInfo, handleObjectChange] = useObjectState<Category>({
    ...
    useEffect(() => {
        const getData = async () => {
            ...
            if (data) {
                ...
                tmp = tmp.filter((x) => x.id !== modalState.data.id);
                const transfer = transformSelect(tmp, 'id', 'name');
                const nullOption: SelectOption = {
                    value: 'none',
                    label: 'None',
                };
                setOptions([nullOption, ...transfer]);
            }
        };
        if (modalState && modalState.visible && modalState.data) {
            const data: Category = modalState.data;
            if (data) {
                setInfo(data);
                getData();
            }
    ...
    const handleChangeSelect = (option: SelectOption) => {
        if (option.value === 'none') {
            setUpdateCategoryInfo('parentId', null);
        } else {
            setUpdateCategoryInfo('parentId', option.value);
        }
    };
    ...
    return (
        <DrawerContainer
            ...
            onClose={() => closeModal(ModalType.UpdateCategory)}
        >
            <div className={'grid items-start gap-4 px-4 md:w-[400px] sm:w-full'}>
                <div className="grid gap-2">
                    ...
                    <Flex items="center" justify="center" className="space-x-2">
                        {files && files.length === 0 && (
                            <img src={updateCategoryInfo.imageFile || Empty} alt="image" className="w-24" />
                        )}
                ...
                <div className="grid gap-2">
                    ...
                    <SingleSelect
                        triggerStyle="w-full"
                        placeHolder={'Please choose parent category ...'}
                        onChange={handleChangeSelect}
                        options={options}
                        defaultValue={updateCategoryInfo.parentId ? updateCategoryInfo.parentId : 'none'}
                    />
                </div>
                <LoadingButton
                    className="mt-2"
